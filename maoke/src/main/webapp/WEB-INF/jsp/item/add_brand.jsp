	...
	$("#form111").submit(function(){
		...
		$(this).find("[reg2]").each(function(){
			...
			if(!reg.test(val)){
				...
				if(name == "brand_name"){
					$.ajax({
						url:"${path}/brand/validateBrandName.controller",
						type:"post",
						dataType:"text",
						data:{
							brandName:val
...
function submitUpload(){
	...
	alert(fileName)
	 var opt = {
		//重新指定form的action的值
		url:"${path}/upload/uploadPic.controller",
		type:"post",
		dateType:"text",
		data:{
			fileName:fileName
...
<div class="frameR"><div class="content">
	...
	<form id="form111" name="form111" action="${path }/brand/addBrand.controller" onsubmit="" method="post" enctype="multipart/form-data">
		<div class="edit set">
			<p><label><samp>*</samp>品牌名称：</label><input type="text" id="brandName" name="brand_name" class="text state" reg2="^[a-zA-Z0-9\u4e00-\u9fa5]{1,20}$" tip="必须是中英文或数字字符，长度1-20"/>
				<span></span>
			</p>
			...
			<p><label class="alg_t">品牌描述：</label><textarea rows="4" cols="45" name="brand_desc" class="are" reg1="^(.|\n){0,300}$" tip="任意字符，长度0-300"></textarea>
				<span class="pos">&nbsp;</span>
			</p>
			<p><label>排序：</label><input type="text" id="brandSort" reg1="^[1-9][0-9]{0,2}$" tip="排序只能输入1-3位数的正整数" name="brand_sort" class="text small"/>
				<span class="pos">&nbsp;</span>
			</p>
